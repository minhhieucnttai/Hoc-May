# -*- coding: utf-8 -*-
"""
Utility: Quick CatBoost Train Script
Huấn luyện nhanh LightGBM (thay thế CatBoost)
"""
...
def quick_train_lightgbm():
    """Train LightGBM model quickly (CatBoost alternative)"""
    
    print("⚡ QUICK TRAIN LIGHTGBM")
    print("="*50)
    ...
    # Save
    with open(Path(__file__).parent / 'lightgbm_model.pkl', 'wb') as f:
        pickle.dump(model, f)
    with open(Path(__file__).parent / 'lightgbm_scaler.pkl', 'wb') as f:
        pickle.dump(scaler, f)
    with open(Path(__file__).parent / 'lightgbm_encoders.pkl', 'wb') as f:
